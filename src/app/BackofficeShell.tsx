...
type Notification = {
  id: string;
  severity: "OVERDUE" | "DUE_SOON" | "WARNING";
  title: string;
  description: string;
  href: string;
};
const groups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/pembelian/retur", icon: "↪", label: "Retur Pembelian" },
      { href: "/retur-penjualan", icon: "↩", label: "Retur Penjualan" },
      { href: "/riwayat-penjualan", icon: "◷", label: "Riwayat Penjualan" },
      { href: "/riwayat-pembelian", icon: "◷", label: "Riwayat Pembelian" },
    ],
  },
  {
    label: "Produk",
    items: [
      { href: "/produk", icon: "▦", label: "Data Produk" },
      { href: "/produk/kategori", icon: "▧", label: "Kategori Produk" },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/stok/riwayat", icon: "◷", label: "Riwayat Stok" },
      { href: "/stok/menipis", icon: "!", label: "Stok Menipis" },
    ],
  },
  {
    label: "Kontak",
    items: [{ href: "/kontak", icon: "◎", label: "Customer & Supplier" }],
  },
  {
    label: "Keuangan",
    items: [
      ...
      {
        href: "/keuangan/pemasukan-pengeluaran",
        icon: "↕",
        label: "Pemasukan & Pengeluaran Kas",
      },
  ...
  },
  {
    label: "Laporan",
    items: [
      { href: "/laporan", icon: "▤", label: "Laporan Kas" },
      { href: "/laporan/penjualan", icon: "▥", label: "Laporan Penjualan" },
      { href: "/laporan/pembelian", icon: "▥", label: "Laporan Pembelian" },
      { href: "/laporan/retur", icon: "↩", label: "Laporan Retur" },
      { href: "/laporan/produk-terlaris", icon: "★", label: "Produk Terlaris" },
      { href: "/piutang", icon: "↗", label: "Piutang dari Customer" },
      { href: "/hutang", icon: "↙", label: "Hutang kepada Supplier" },
    ],
  },
  {
    label: "Sistem",
...
export default function BackofficeShell({
  ...
  const darkMode = useSyncExternalStore(
    (onStoreChange) => {
      ...
      return () =>
        window.removeEventListener("kasir-toko-theme-change", onStoreChange);
    },
    () => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("kasir-toko-profile-change", refreshOperator);
  }, []);
  ...
  async function changeAvatar(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || "Foto profil gagal disimpan.");
      setOperator(data.operator);
      window.dispatchEvent(new Event("kasir-toko-profile-change"));
    } catch (error) {
      window.alert(
        error instanceof Error ? error.message : "Foto profil gagal disimpan.",
      );
    } finally {
  ...
  function isActive(href: string) {
    ...
    const pathMatches =
      pathname === itemPath || (itemPath === "/" && pathname === "/");
  ...
  return (
    <div
      className={`backoffice-layout ${sidebarIsCollapsed ? "sidebar-collapsed" : ""} ${pinned ? "sidebar-pinned" : ""}`}
    >
      <header ref={globalHeaderRef} className="backoffice-global-header">
        <button
          ...
          type="button"
          aria-label={
            sidebarIsCollapsed ? "Lebarkan sidebar" : "Minimalkan sidebar"
          }
        ...
        <Link
          className="backoffice-brand"
          href="/dashboard"
          onClick={openSidebarFromBrandClick}
          title="Dashboard"
        >
          <span className="backoffice-brand-mark" aria-hidden="true">
            <i>K</i>
            <i>T</i>
          </span>
          <span className="backoffice-brand-name">
            <b>Kasir Toko</b>
          </span>
        </Link>
        {pinned && (
          <button
            className="sidebar-pin active"
            type="button"
            onClick={togglePinned}
            aria-label="Lepas pin sidebar"
            title="Lepas pin sidebar"
          >
            <svg viewBox="0 0 24 24" aria-hidden="true">
              <path d="m14.5 3 6.5 6.5-2.2 2.2-1.8-1.8-3.7 3.7 1.8 1.8-2.2 2.2-6.5-6.5 2.2-2.2 1.8 1.8 3.7-3.7-1.8-1.8L14.5 3Z" />
              <path d="m10.8 13.2-7.3 7.3M3.5 20.5l3 .1" />
            </svg>
          </button>
        )}
              ...
              {notifications.length ? (
                notifications.map((notification) => (
                  <Link
                    ...
                    className={`notification-item notification-${notification.severity.toLowerCase()}`}
                    key={notification.id}
          ...
          >
            <div className="backoffice-avatar">
              {operator?.avatarUrl ? (
                ...
                operator?.name?.slice(0, 2).toUpperCase() || "TB"
              )}
            </div>
            <span>
              <b>{operator?.name || "Operator"}</b>
              <small>{operator?.role || "Memuat..."}</small>
            </span>
          </button>
          {userMenuOpen && (
      ...
      <div className="backoffice-body">
        ...
        >
          <div
            className={`backoffice-sidebar-header ${pinned ? "sidebar-header-pinned" : ""}`}
          >
            <button
              className="sidebar-toggle"
              type="button"
            ...
            <Link
              className="backoffice-brand"
              href="/dashboard"
              onClick={closeSidebarAfterBrandClick}
              title="Dashboard"
            >
              <span className="backoffice-brand-mark" aria-hidden="true">
                <i>K</i>
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span>{item.icon}</span>
                      <b>
                        <em>{shortMenuLabel(item.label)}</em>
                        <i>{item.label}</i>
                      </b>
                    </Link>
                  );
          ...
          <div className="backoffice-footer">
            <Link
              ...
              href="/pengaturan"
              ref={isActive("/pengaturan") ? activeLinkRef : undefined}
              title={sidebarIsCollapsed ? "Setting" : undefined}
              onClick={minimizeSidebar}
            >
              <span>⚙</span>
              <b>
          ...
          >
            ...
            >
              <div className="modal-heading">
                <div>
                  <p className="eyebrow">AKUN OPERATOR</p>
                  <h2 id="profile-title">Profil Saya</h2>
                </div>
                <button
              ...
              <div className="profile-modal-body">
                ...
                <div>
                  <h3>{operator?.name || "Operator"}</h3>
                  <p>
                    Email / username:{" "}
                    {operator?.email || operator?.username || "—"}
                  </p>
                  <p>
                    Peran: <b>{operator?.role || "—"}</b>
                  </p>
                  <label className="profile-upload">
                    {avatarSaving ? "Menyimpan..." : "Ganti foto profil"}
                    <input
                      type="file"
                  ...
                  <small>JPG, PNG, atau WebP maksimal 1,5 MB.</small>
                </div>
              </div>
            </section>
          </div>
        ...
        {logoutConfirmOpen && (
          ...
          >
            ...
            >
              ...
              <p>Anda akan keluar dari sesi Kasir Toko.</p>
