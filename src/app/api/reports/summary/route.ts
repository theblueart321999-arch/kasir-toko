...
export async function GET(request: NextRequest) {
  ...
  try {
    const [sales, purchases, saleReturns, purchaseReturns, movements, lowStock, paymentBreakdown, purchasePaymentBreakdown, topProducts, saleRows, purchaseRows, saleReturnRows, purchaseReturnRows, movementRows] = await Promise.all([
      ...
      prisma.saleItem.groupBy({ by: ["productId"], where: { sale: { createdAt: where, status: "COMPLETED" } }, _sum: { quantity: true, total: true }, orderBy: { _sum: { quantity: "desc" } }, take: 100 }),
      ...
      prisma.saleReturn.findMany({ where: { createdAt: where, status: "COMPLETED" }, select: { total: true, createdAt: true }, orderBy: { createdAt: "asc" } }),
      prisma.purchaseReturn.findMany({ where: { createdAt: where, status: "COMPLETED" }, select: { total: true, createdAt: true }, orderBy: { createdAt: "asc" } }),
      prisma.moneyMovement.findMany({ where: { createdAt: where }, select: { type: true, amount: true, operator: { select: { name: true } } } }),
    ]);
    const products = await prisma.product.findMany({ where: { id: { in: topProducts.map((p) => p.productId) } }, select: { id: true, name: true, sku: true, unit: true } });
