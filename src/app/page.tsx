...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetch("/api/products"), fetch("/api/categories")])
      .then(async ([productsResponse, categoriesResponse]) => {
        ...
        const apiCategories = categoriesResponse.ok
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      .then(async (response) => {
        ...
        if (typeof data.operator?.name === "string")
    ...
    try {
      const response = await fetch(`/api/products/${editingProduct.id}`, {
        ...
        body: JSON.stringify({
          ...
          category:
            editingProduct.category === "Bangunan" ? "BANGUNAN" : "PERTANIAN",
        }),
      });
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.error || "Produk gagal diperbarui");
  ...
  function changeQuantity(id: number, delta: number) {
    ...
    fetch("/api/settings/store")
      ...
      .then((data) => {
        if (data && typeof data.taxRate === "number") setTaxRate(data.taxRate);
      })
      .catch(() => undefined);
  }
    ...
    try {
      ...
      if (refreshed.ok) {
        const data = normalizeProducts(
          (await refreshed.json()) as ApiProduct[],
        );
    ...
    <main className="app-shell">
      <section className="workspace" id="kasir">
        <header className="topbar">
          <div>
            <p className="eyebrow">SABTU, 19 SEPTEMBER 2026</p>
            <h1>
              Selamat datang{operatorName ? `, ${operatorName}` : ""}{" "}
              <span>✦</span>
            </h1>
          </div>
        </header>
        <div className="content-grid">
          ...
          <aside className={`cart-panel ${showCart ? "cart-panel-open" : ""}`}>
            <div className="cart-heading">
              <div>
                <h2>Keranjang</h2>
                <p>{itemCount} item dalam transaksi</p>
              </div>
              <div className="cart-heading-actions">
                <button
                  ...
                  onClick={clearCart}
                >
                  <span aria-hidden="true">⌫</span> Kosongkan
                </button>
                <button
              ...
              {cartItems.length ? (
                cartItems.map((product) => (
                  <div className="cart-item" key={product.id}>
                    <div className={`mini-icon ${product.color}`}>
                      {product.icon}
                    </div>
                    ...
                    <button
                      ...
                      onClick={() =>
                        changeQuantity(product.id, -cart[product.id])
                      }
                    >
                      ×
                    </button>
      ...
      {showPaymentDetails && (
        <div className="payment-detail-backdrop">
          <section
            className="payment-detail-page"
          ...
          >
            <header className="payment-detail-header">
              ...
              <div>
                <p className="eyebrow">KONFIRMASI TRANSAKSI</p>
            ...
            <div className="payment-detail-content">
              <div className="payment-order-card">
                ...
                {cartItems.map((product) => (
                  <div className="payment-order-line" key={product.id}>
                    <span>
                      {product.name} <small>× {cart[product.id]}</small>
                    </span>
                    <b>
                      {formatCurrency(
                        Math.round(
              ...
              <div className="payment-choice-card">
                ...
                <p>Metode pembayaran akan digunakan untuk transaksi ini.</p>
                <div className="payment-method payment-method-floating">
                ...
                {payment === "Tunai" && (
                  <div className="cash-form">
                    ...
                    <div className="cash-change">
                      <span>Kembalian</span>
                      <strong className={received < total ? "not-enough" : ""}>
                        {received >= total
                ...
                {payment === "QRIS" && (
                  <div className="payment-instruction">
                    <span>▦</span>
                    <div>
                      <b>QRIS siap digunakan</b>
                      <p>Tampilkan kode QR saat konfirmasi pembayaran.</p>
                    </div>
                  </div>
                )}
                {payment === "Debit" && (
                  <div className="payment-instruction">
                    ...
                    <div>
                      <b>Mesin EDC</b>
                      <p>
                        Pastikan pembayaran kartu berhasil sebelum konfirmasi.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="payment-confirm-bar">
              <div>
                <span>Total yang harus dibayar</span>
                <strong>{formatCurrency(total)}</strong>
              </div>
              <button
                disabled={payment === "Tunai" && received < total}
                onClick={completeSale}
              >
                Konfirmasi Pembayaran <span>→</span>
              </button>
            </div>
          </section>
        </div>
      ...
      <button
        className={`floating-cart ${itemCount ? "has-items" : ""}`}
      ...
      >
        ...
        <span className="floating-cart-label">
          {itemCount ? `${formatCurrency(total)}` : "Keranjang"}
        </span>
        ...
        <div className="modal-backdrop">
          <form className="product-form" onSubmit={saveProductEdit}>
            <div className="modal-heading">
              <div>
                <p className="eyebrow">EDIT PRODUK</p>
            ...
            <div className="form-row">
              ...
              <label>
                ...
                <input
                  ...
                  value={editForm.unit}
                  onChange={(event) =>
                    setEditForm({ ...editForm, unit: event.target.value })
                  }
                />
              </label>
            </div>
            ...
            <div className="form-actions">
              ...
              <button className="management-primary" disabled={savingProduct}>
                {savingProduct ? "Menyimpan..." : "Simpan perubahan"}
              </button>
            </div>
          </form>
        </div>
      ...
      {receiptId && (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setReceiptId(null);
          }}
        >
          <section
            className="receipt-success-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="modal-heading">
              <div>
                ...
                <h2 id="receipt-success-title">Cetak struk sekarang?</h2>
              </div>
              <button
                className="modal-close"
                type="button"
            ...
            <p className="receipt-success-message">
              {notice || "Transaksi berhasil disimpan."}
            </p>
            <div className="form-actions">
              <button
                className="modal-cancel"
