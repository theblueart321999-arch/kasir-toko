...
type ReturnRecord = {
  ...
  reason: string;
  createdAt: string;
  purchase: { invoiceNumber: string; supplier: Supplier };
  items: PurchaseItem[];
};
...
export default function PembelianManager({
  ...
  operatorName: string;
  initialTab?: "purchase" | "return" | "history";
}) {
  ...
  const [returnForm, setReturnForm] = useState({
    purchaseId: "",
    reason: "",
    itemQuantities: {} as Record<number, string>,
  });
  ...
  const selectedPurchase = purchases.find(
    (item) => item.id === Number(returnForm.purchaseId),
  );
  ...
  function historyRange() {
    ...
    if (historyPeriod === "day") {
      ...
      const [startYear, startMonth, startDay] = historyStart
        .split("-")
        .map(Number);
  ...
  const isInHistoryRange = (value: string) =>
    !range || (new Date(value) >= range.start && new Date(value) <= range.end);
  const filteredPurchases = purchases.filter((item) =>
    isInHistoryRange(item.createdAt),
  );
  ...
  function selectProduct(index: number, value: string) {
    ...
    items[index] = {
      ...
      unitPrice:
        products
          .find((product) => product.id === Number(value))
          ?.price.toString() || "",
    };
  ...
  async function createProduct() {
    const unit =
      newProduct.unit === "manual"
        ? newProduct.manualUnit.trim()
        : newProduct.unit;
    ...
    const convertedPrice = newProduct.convertToRupiah
      ? Math.round(sourcePrice * rate)
      : sourcePrice;
    if (
  ...
  async function createSupplier() {
    // Hanya nama supplier yang wajib diisi
        ...
        paidAmount:
          purchase.paymentStatus === "PARTIAL"
            ...
            : purchase.paymentStatus === "PAID"
              ...
              : 0,
        dueDate:
          purchase.paymentStatus === "PAID" ? undefined : purchase.dueDate,
  ...
  async function submitReturn(event: FormEvent) {
    ...
    const items = selectedPurchase.items
      .map((item) => ({
        ...
        quantity: Number(returnForm.itemQuantities[item.productId] || 0),
      }))
    ...
    const data = await response.json();
    if (!response.ok)
      return message(data.error || "Retur pembelian gagal disimpan");
    setReturnForm({ purchaseId: "", reason: "", itemQuantities: {} });
  ...
  return (
    <main className="purchase-page">
      <header className="management-header">
        <div>
          <p className="eyebrow">KASIR TOKO · PEMBELIAN</p>
          <h1>
            {tab === "history"
              ? "Riwayat Pembelian"
              : tab === "return"
          ...
          <p>
            {tab === "history"
              ? "Lihat dan filter seluruh riwayat pembelian."
              : tab === "return"
                ? "Proses pengembalian barang kepada supplier."
                : "Catat penerimaan barang baru dari supplier."}
          </p>
        </div>
        <div className="management-user">
          Masuk sebagai <b>{operatorName}</b>
      ...
      {tab === "purchase" && (
        <form className="purchase-card" onSubmit={submitPurchase}>
          <div className="card-title">
            <div>
              <h2>Pembelian dari supplier</h2>
              <p>Stok otomatis bertambah dan tercatat sebagai stok masuk.</p>
            </div>
            <span />
          </div>
          <div className="purchase-fields">
            <label>
              Supplier <span className="optional-label">(opsional)</span>
              <select
                value={purchase.supplierId}
                onChange={(e) => selectSupplier(e.target.value)}
              >
                <option value="">Kosong — buat anonim otomatis</option>
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setPurchase({ ...purchase, invoiceNumber: e.target.value })
                }
                    ...
                    paidAmount:
                      e.target.value === "PAID"
                        ...
                        : e.target.value === "UNPAID"
                          ? "0"
                          : purchase.paidAmount,
                    dueDate: e.target.value === "PAID" ? "" : purchase.dueDate,
                  })
                }
              >
            ...
            {purchase.paymentStatus === "PARTIAL" && (
              <label>
                ...
                <input
                  required
                  min="1"
                  max={purchaseTotal > 0 ? purchaseTotal - 1 : undefined}
                  type="number"
                  value={purchase.paidAmount}
                  onChange={(e) =>
                    setPurchase({ ...purchase, paidAmount: e.target.value })
                  }
                  placeholder="Nominal yang sudah dibayar"
                />
                <small>Sisa otomatis masuk hutang.</small>
              </label>
            )}
            {purchase.paymentStatus !== "PAID" && (
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setPurchase({ ...purchase, dueDate: e.target.value })
                  }
                />
                <small>Tanggal pembayaran terakhir kepada supplier.</small>
              </label>
            )}
          </div>
          <div className="purchase-actions">
            <button type="submit" className="management-primary">
              Simpan pembelian
            </button>
          </div>
        </form>
      ...
      {tab === "return" && (
        <form className="purchase-card" onSubmit={submitReturn}>
          <div className="card-title">
            <div>
              <h2>Retur pembelian</h2>
              <p>Jumlah retur tidak boleh melebihi sisa jumlah pembelian.</p>
            </div>
          </div>
          <label>
            Pembelian
            ...
            >
              ...
              {purchases.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.invoiceNumber} · {item.supplier.name} ·{" "}
                  {money(item.total)}
                </option>
              ))}
            </select>
              ...
              {selectedPurchase.items.map((item) => (
                <label key={item.productId}>
                  <span>
                    ...
                    <small>
                      Dibeli {item.quantity} {item.product.unit}
                    </small>
                  </span>
                  <input
                    type="number"
                    min="0"
                    max={item.quantity}
                    value={returnForm.itemQuantities[item.productId] || ""}
                    onChange={(e) =>
                      setReturnForm({
                        ...returnForm,
                        itemQuantities: {
          ...
          <button className="management-primary" type="submit">
            Simpan retur
          </button>
        </form>
      )}
        ...
        <section className="purchase-history">
          <div className="card-title">
            <div>
              ...
              <p>
                {filteredPurchases.length} pembelian · {filteredReturns.length}{" "}
                retur
              </p>
            </div>
          </div>
            ...
            {pagedPurchases.map((item) => (
              <article key={item.id}>
                <div>
                  ...
                  <span>
                    {item.supplier.name} · {date(item.createdAt)}
                  </span>
                </div>
          ...
          />
          <h2 className="history-subtitle">Retur pembelian</h2>
          <div className="purchase-history-list">
            {pagedReturns.map((item) => (
              <article key={item.id}>
                <div>
                  <b>{item.purchase.invoiceNumber}</b>
                  <span>
                    {item.purchase.supplier.name} · {date(item.createdAt)}
                  </span>
                </div>
                <strong>{money(item.total)}</strong>
        ...
        >
          ...
          >
            <div className="modal-heading">
              <div>
                <p className="eyebrow">DATA SUPPLIER</p>
                <h2 id="supplier-dialog-title">Tambah supplier baru</h2>
              </div>
              <button
                type="button"
                className="modal-close"
                aria-label="Tutup tambah supplier"
                onClick={() => setSupplierDialogOpen(false)}
              >
                ×
              </button>
            </div>
            <p className="supplier-required-note">
            ...
            <div className="form-row">
              <label>
                No. telepon <span className="optional-label">(opsional)</span>
                <input
              ...
              <label>
                Email <span className="optional-label">(opsional)</span>
                <input
                  type="email"
            ...
            <label>
              Alamat <span className="optional-label">(opsional)</span>
              <textarea
                rows={2}
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) =>
                  setNewSupplier({ ...newSupplier, notes: event.target.value })
                }
      ...
      {productDialogOpen && (
        ...
        >
          ...
          >
            <div className="modal-heading">
              <div>
                <p className="eyebrow">DATA PRODUK</p>
                <h2 id="product-dialog-title">Tambah produk baru</h2>
              </div>
              <button
                type="button"
                className="modal-close"
            ...
            <p className="supplier-required-note">
              SKU kosong dibuat otomatis. Harga akan disimpan dalam Rupiah.
            </p>
            <label>
              Gambar produk{" "}
              <input
            ...
            <fieldset className="unit-choice">
              ...
              <div>
                {["pcs", "kg", "sak", "liter", "meter", "kodi", "manual"].map(
                  (unit) => (
                    <label key={unit}>
                      <input
                        type="radio"
                        name="product-unit"
                        checked={newProduct.unit === unit}
                        onChange={() => setNewProduct({ ...newProduct, unit })}
                      />
                      {unit === "manual" ? "Isi manual" : unit}
                    </label>
                  ),
                )}
              </div>
            ...
            {newProduct.currency !== "IDR" && (
              <>
                <label className="checkbox-line">
                  <input
                    type="checkbox"
                    checked={newProduct.convertToRupiah}
                    onChange={(event) =>
                      setNewProduct({
                        ...newProduct,
                        convertToRupiah: event.target.checked,
                      })
                    }
                  />{" "}
                  Konversi ke Rupiah saat disimpan
                </label>
                {newProduct.convertToRupiah && (
                  <label>
                    Kurs 1 {newProduct.currency} ke Rupiah *
                    <input
                      required
                      min="0.01"
                      step="0.01"
