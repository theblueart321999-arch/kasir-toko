...
export default function StokManager({ operatorName }: { operatorName: string }) {
  ...
  return <main className="stock-page">
    <header className="management-header"><div><p className="eyebrow">TANIBANGUN POS · INVENTORI</p><h1>Manajemen Stok</h1><p>Pantau persediaan dan catat setiap perubahan stok.</p></div><div className="management-user">Masuk sebagai <b>{operatorName}</b><a href="/dashboard">← Dashboard</a></div></header>
    ...
    <section className="product-table-card stock-history"><div className="card-title"><div><h2>Riwayat perubahan stok</h2><p>100 aktivitas terbaru</p></div></div><div className="product-table-wrap"><table className="product-table"><thead><tr><th>Waktu</th><th>Produk</th><th>Perubahan</th><th>Stok</th><th>Operator</th></tr></thead><tbody>{movements.map((m) => <tr key={m.id}><td>{date(m.createdAt)}</td><td><b>{m.product.name}</b><small>{m.note || "Tanpa catatan"}</small></td><td><span className={m.type === "OUT" ? "movement-out" : "movement-in"}>{m.type === "OUT" ? "-" : "+"}{m.quantity} · {labels[m.type]}</span></td><td>{m.beforeStock} → <b>{m.afterStock}</b></td><td>{m.operator.name}</td></tr>)}</tbody></table>{!movements.length && <p className="management-empty">Belum ada perubahan stok.</p>}</div></section>
  </main>;
}
