...
export default function StokManager({ operatorName }: { operatorName: string }) {
  ...
  useEffect(() => { setProductPage(1); setHistoryPage(1); }, [query, lowOnly]);
  const pagedProducts = useMemo(() => paginate(visible, productPage, pageSize), [visible, productPage, pageSize]);
  const pagedMovements = useMemo(() => paginate(movements, historyPage, pageSize), [movements, historyPage, pageSize]);
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch("/api/inventory/movements", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, productId: Number(form.productId), quantity: Number(form.quantity) }) });
    const data = await response.json();
    if (!response.ok) return message(data.error || "Perubahan stok gagal disimpan");
    setForm({ ...form, quantity: "", note: "" }); message("Perubahan stok berhasil disimpan"); void load();
  }
  return <main className="stock-page">
    ...
    <section className="stock-summary"><article><span>Total produk</span><strong>{products.length}</strong><small>Produk aktif di katalog</small></article><article><span>Stok menipis</span><strong className={lowCount ? "stock-low" : ""}>{lowCount}</strong><small>Di bawah 10 unit</small></article><article><span>Aktivitas terbaru</span><strong>{movements.length}</strong><small>Perubahan stok tercatat</small></article></section>
    <section className="stock-layout"><form className="stock-form-card" onSubmit={submit}><div className="card-title"><div><h2>Catat perubahan</h2><p>Stok diperbarui secara atomik.</p></div></div><label>Produk<select required value={form.productId} onChange={(e) => setForm({ ...form, productId: e.target.value })}><option value="">Pilih produk</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name} ({p.stock} {p.unit})</option>)}</select></label><label>Jenis perubahan<select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}><option value="IN">{labels.IN}</option><option value="OUT">{labels.OUT}</option><option value="ADJUSTMENT">{labels.ADJUSTMENT}</option></select></label><label>Jumlah<input required min="1" type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /></label><label>Catatan (opsional)<textarea value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Contoh: Restock dari supplier" rows={3} /></label><button className="management-primary" type="submit">Simpan perubahan</button></form>
    ...
    <section className="product-table-card stock-history"><div className="card-title"><div><h2>Riwayat perubahan stok</h2><p>100 aktivitas terbaru</p></div></div><div className="product-table-wrap"><table className="product-table"><thead><tr><th>Waktu</th><th>Produk</th><th>Perubahan</th><th>Stok</th><th>Operator</th></tr></thead><tbody>{pagedMovements.map((m) => <tr key={m.id}><td>{date(m.createdAt)}</td><td><b>{m.product.name}</b><small>{m.note || "Tanpa catatan"}</small></td><td><span className={m.type === "OUT" ? "movement-out" : "movement-in"}>{m.type === "OUT" ? "-" : "+"}{m.quantity} · {labels[m.type]}</span></td><td>{m.beforeStock} → <b>{m.afterStock}</b></td><td>{m.operator.name}</td></tr>)}</tbody></table>{!movements.length && <p className="management-empty">Belum ada perubahan stok.</p>}<Pagination page={historyPage} pageSize={pageSize} total={movements.length} onPageChange={setHistoryPage} onPageSizeChange={(size) => { setPageSize(size); setProductPage(1); setHistoryPage(1); }} /></div></section>
  </main>;
}
