...
export default function StockHistoryManager({ operatorName }: { operatorName: string }) {
  ...
  return <main className="management-page">
    ...
    <section className="product-table-card stock-history-page-card"><div className="card-title stock-history-heading"><div><h2>Riwayat perubahan stok</h2><p>{visible.length} aktivitas ditampilkan</p></div><div className="stock-history-filters"><div className="management-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari produk, SKU, atau catatan..." /></div><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="">Semua kategori</option>{categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div></div>
      <div className="product-table-wrap"><table className="product-table"><thead><tr><th>Waktu</th><th>Produk</th><th>Kategori</th><th>Perubahan</th><th>Stok</th><th>Operator</th></tr></thead><tbody>{paged.map((movement) => { const isDecrease = movement.afterStock < movement.beforeStock; const sign = isDecrease ? "-" : "+"; const movementLabel = movement.type === "ADJUSTMENT" ? `${labels.ADJUSTMENT} (${sign})` : labels[movement.type]; return <tr key={movement.id}><td>{formatDate(movement.createdAt)}</td><td><b>{movement.product.name}</b><small>{movement.product.sku} · {movement.note || "Tanpa catatan"}</small></td><td>{movement.product.categoryRef?.name || "Tanpa kategori"}</td><td><span className={isDecrease ? "movement-out" : "movement-in"}>{sign}{movement.quantity} · {movementLabel}</span></td><td>{movement.beforeStock} → <b>{movement.afterStock}</b> {movement.product.unit}</td><td>{movement.operator.name}</td></tr>; })}</tbody></table>{!visible.length && <p className="management-empty">{query || category ? "Riwayat stok tidak ditemukan." : "Belum ada perubahan stok."}</p>}</div>
